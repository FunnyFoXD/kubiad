...
func (r *Reconciler) Reconcile(ctx context.Context, request ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil { return ctrl.Result{}, err }
	port, err := int32Value(application, "port", 8080)
	if err != nil { return ctrl.Result{}, err }
	...
	if _, err := controllerutil.CreateOrUpdate(ctx, r.Client, deployment, func() error {
		...
		deployment.Spec.Template.Spec.Containers = []corev1.Container{{Name: "application", Image: image, Ports: []corev1.ContainerPort{{Name: "application", ContainerPort: port}}}}
	...
	if _, err := controllerutil.CreateOrUpdate(ctx, r.Client, service, func() error {
		...
		service.Spec.Ports = []corev1.ServicePort{{Name: "application-service", Port: port, TargetPort: intstr.FromInt32(port)}}
